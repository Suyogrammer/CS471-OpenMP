program hwk6
  ...
  implicit none
  integer :: nr,ns,i,j,minGrid,errCheck,l, mweak, timed_loops
  real(kind = 8) :: time1a,time2a
  real(kind = 8), dimension(:), allocatable :: timediff
  real(kind = 8) :: hr,hs, detMat, JacobianMax
  real(kind = 8), dimension(:), allocatable :: r,s,h_eff,errValue
  ...
  allocate(h_eff(0:errCheck),errValue(0:errCheck))
  ...
  ! I need to leave this part serial because each value is needed for
  ! each other.
  ...
  do l = 1,timed_loops
     ...
     do i = 0,errCheck
        h_eff(i)=0
       errValue(i)=0
       call error(i+minGrid,i+minGrid,i,h_eff,errValue,errCheck)
    end do
    !$OMP END PARALLEL DO 
    !$ time2a = omp_get_wtime()
    !$ timediff(l-1)=time2a-time1a
 end do
 ! this wil print a file where the 1st entry corisponds to 1 thread the
 ! 2nd to 2 ...
 !$ call  printdble2d(timediff,timed_loops-1,0,'time.txt')
 call  printdble2d(h_eff,errCheck,0,'h_eff.txt')
 call  printdble2d(errValue,errCheck,0,'errValue.txt')
...
!$     h_eff(i)=0
!$     errValue(i)=0
!$     call error(20,20,i,h_eff,errValue,errCheck)
!$   end do
     !$OMP END PARALLEL DO 
!$ time2a = omp_get_wtime()
!$ timediff(l-1)=time2a-time1a
!$ end do
!!!!! this wil print a file where the 1st entry corresponds to 1 thread
!the 2nd to 2 ...
!$ call  printdble2d(timediff,timed_loops-1,0,'strong20.txt')
...
!$        h_eff(i)=0
!$       errValue(i)=0
!$       call error(800,800,i,h_eff,errValue,errCheck)
!$    end do
     !$OMP END PARALLEL DO 
!$    time2a = omp_get_wtime()
!$    timediff(l-1)=time2a-time1a
!$  end do
!!!!! this wil print a file where the 1st entry corisponds to 1 thread
!the 2nd to 2 ...
!$ call  printdble2d(timediff,timed_loops-1,0,'strong800.txt')
...
!$        h_eff(i)=0
!$        errValue(i)=0
!$       call error(mweak,mweak,i,h_eff,errValue,errCheck)
!$    end do
    !$OMP END PARALLEL DO 
    !$ time2a = omp_get_wtime()
    !$ timediff(l-1)=time2a-time1a
